...
export class AuditQueryHelper {
    ...
    public static async auditPostInsert(dbcli, idtabla, idusuario, id) {
        const queryAudit: string = `INSERT INTO public.evento_auditoria(id, fecha_hora, idusuario, idtabla, operacion, estado_nuevo)
            VALUES(nextval('evento_auditoria_seq'), NOW(), $1, $2, 'R', (SELECT row_to_json(${TablasAuditoriaList.getTableName(idtabla)}) FROM public.${TablasAuditoriaList.getTableName(idtabla)} WHERE id = $3))`;
    ...
    public static async auditPreUpdate(dbcli, idtabla, idusuario, oldid): Promise<number> {
        const queryAudit: string = `INSERT INTO public.evento_auditoria(id, fecha_hora, idusuario, idtabla, operacion, estado_anterior)
            VALUES(nextval('evento_auditoria_seq'), NOW(), $1, $2, 'M', (SELECT row_to_json(${TablasAuditoriaList.getTableName(idtabla)}) FROM public.${TablasAuditoriaList.getTableName(idtabla)} WHERE id = $3)) RETURNING *`;
    ...
    public static async auditPostUpdate(dbcli, idtabla, idevento, newid){
        const queryAuditU: string = `UPDATE public.evento_auditoria SET estado_nuevo = (SELECT row_to_json(${TablasAuditoriaList.getTableName(idtabla)}) FROM public.${TablasAuditoriaList.getTableName(idtabla)} WHERE id = $1) WHERE id = $2`;
    ...
    public static async auditPostDelete(dbcli, idtabla, idusuario, id){
        const queryAuditI: string = `INSERT INTO public.evento_auditoria(id, fecha_hora, idusuario, idtabla, operacion, estado_anterior)
            VALUES(nextval('evento_auditoria_seq'), NOW(), $1, $2, 'E', (SELECT row_to_json(${TablasAuditoriaList.getTableName(idtabla)}) FROM public.${TablasAuditoriaList.getTableName(idtabla)} WHERE id = $3)) RETURNING *`;
